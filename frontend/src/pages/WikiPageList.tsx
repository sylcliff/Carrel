        ...
        <>
          ...
          {loading ? <p className="text-sm text-muted-foreground">Loading…</p> : pages.length === 0 ? <p className="rounded-lg border border-dashed p-5 text-sm text-muted-foreground">No {kind} pages yet.</p> : (
            <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
              {pages.map((page) => <Link key={page.id} to={`/wiki/${kind}/${page.slug}`}>
                <Card className="h-full transition-colors hover:bg-muted/30"><CardContent className="space-y-3 p-5">
                  <div className="flex items-start justify-between gap-3"><h2 className="font-semibold">{page.title}</h2><span className="rounded-full bg-muted px-2 py-0.5 text-xs tabular-nums">{Math.round(page.confidence * 100)}%</span></div>
